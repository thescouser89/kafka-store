...
class BuildStageRecordTest {
    ...
    void testGetForBuildConfigId() {
        ...
        assertThat(result.keySet()).as("check if result contains proper build id size").hasSize(2);
        Assertions.assertThat(result.get(buildId)).as("check if each build id has the proper number of stages")
                .hasSize(11);
        Assertions.assertThat(result.get(buildIdSecond)).as("check if each build id has the proper number of stages")
                .hasSize(7);

        Map<String, List<BuildStageRecord>> resultOne = BuildStageRecord.getForBuildConfigId(buildConfigId, 1);
        assertThat(resultOne.keySet()).as("check if result returns only 1 build id").hasSize(1);
        Assertions.assertThat(resultOne.get(buildIdSecond)).as("check if each build id has the proper number of stages")
                .isNotNull().hasSize(7);

        Map<String, List<BuildStageRecord>> buildConfigSecondResult = BuildStageRecord
                .getForBuildConfigId(buildConfigIdSecond, 5);

        assertThat(buildConfigSecondResult.keySet()).as("check if result contains proper build id size").hasSize(1);
        Assertions.assertThat(buildConfigSecondResult.get(buildIdThird))
                .as("check if each build id has the proper number of stages").hasSize(8);
    }
